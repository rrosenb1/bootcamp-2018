...
setwd("/nfs/home/rra9741/R_sessions")
...
avg <- mean(gapminder07$lifeExp)
gapminder07$lifeExp_highlow <- NA
gapminder07$lifeExp_highlow[gapminder07$lifeExp > avg] <- "High"
gapminder07$lifeExp_highlow[gapminder07$lifeExp < avg] <- "Low"
...
aggregate(gapminder07$lifeExp, list(gapminder07$continent), mean)
# can also use ~ to group
...
# Run summary() on reg1.
summary(reg1)
# gives more detail - std error, t value, p value, etc
...
write.csv(gapminder07, file = "data/gapminder07", row.names = FALSE)
...
hist(gapminder07$lifeExp, main = "Life Expectancy Across Countries in 2007", xlab = "LifeExp", ylab = "# in bin")

# Bonus: Change the `breaks = ` argument from its default setting and see what happens.
hist(gapminder07$lifeExp, main = "Life Expectancy", xlab = "LifeExp", ylab = "Frequency", breaks = 20)
...
plot(gapminder07$lifeExp, gapminder07$gdpPercap, main = "Life expectancy versus GDP", xlab = "Life Expectancy", ylab = "GDP per capita")

# Bonus: Add a horizontal line indicating the mean of `lifeExp` onto the plot using `abline()`.
plot(gapminder07$gdpPercap, gapminder07$lifeExp, main = "Life expectancy versus GDP", xlab = "Life Expectancy", ylab = "GDP per capita")
abline(mean(gapminder07$lifeExp), 0)
